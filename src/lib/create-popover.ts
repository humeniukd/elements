...
export function createPopover(
    ...
    el.addEventListener("beforetoggle", evt => {
        ...
        anchorUpdater(evt.newState === "open", getAnchor?.() ?? anchor);
        ...
        if (evt.newState === "open" && !isOpen) {
            el.setAttributeNoCallbacks("open", "");
        } else if (evt.newState === "closed" && isOpen) {
            el.removeAttributeNoCallbacks("open");
        }
        if (evt.newState === "open") {
        ...
        if (evt.oldState === "closed" && evt.newState === "open") {
            ...
            if (isSafari)
                d.style(el, "transition-property", "none");
            t.start("out");
        }
    }, { signal });
    signal.addEventListener("abort", () => t.abort());
}