...
export function handleIOSLocking(): ScrollLockStep {
  if (!isIOS()) {
    return {}
  }
  ...
  return {
    before({ doc, d}) {
      function inAllowedContainer(el: Element) {
        return Boolean(el.closest('[popover], dialog > *'));
      }
      ...
      d._microTask(() => {
        ...
        if (window.getComputedStyle(doc.documentElement).scrollBehavior !== 'auto') {
          let _d = disposables()
          _d._style(doc.documentElement, 'scrollBehavior', 'auto')
          d._add(() => d._microTask(() => _d._dispose()))
        }
        ...
        // exists an element on the page (outside the Dialog) with that id, then the browser will
        ...
        // Rely on overscrollBehavior to prevent scrolling outside the Dialog.
        d._group((_d) => {
          d._addEventListener(doc, 'touchstart', (e) => {
            _d._dispose()
            ...
            if (DOM.isHTMLorSVGElement(e.target) && DOM.hasInlineStyle(e.target)) {
              if (inAllowedContainer(e.target)) {
                ...
                _d._style(rootContainer, 'overscrollBehavior', 'contain')
              } else {
                _d._style(e.target, 'touchAction', 'none')
              }
            }
          })
        })
