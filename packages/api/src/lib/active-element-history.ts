...
onDocumentReady(() => {
    ...
    document.body.addEventListener('pointerdown', handle, { capture: true })
    document.body.addEventListener('focus', handle, { capture: true })
})


export function findEventTarget(evt: ToggleEvent, list: HTMLElement[]): HTMLElement | null {
    // @ts-expect-error
    return evt.source ??
        history.find(el => list.includes(el)) ??
        list[0] ?? null;
}