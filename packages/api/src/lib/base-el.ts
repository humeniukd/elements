...
export abstract class BaseElement extends HTMLElement {
    ...
    protected _mount?(signal: AbortSignal): void;
    protected _onAttributeChange?(name: string, oldValue: string, newValue: string): void;
    ...
    connectedCallback() { // @ts-expect-error
        ...
        queueMicrotask(() => {
            ...
            try {
                this._mount?.(this.#abortCtrl.signal);
            } catch (e) {
                console.error(e);
            }
        });
    ...
    _setAttributeNoCallbacks(name: string, value: string) {
        try {
    ...
    _removeAttributeNoCallbacks(name: string) {
        try {
            this.#disableObservation = true;
